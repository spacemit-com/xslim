...
| `opset` | `int` | 保持输入模型原有默认 ai.onnx opset | 在精简、量化和导出期间把默认 ai.onnx opset 转换到目标版本 |
...
> `opset` 是命令行 `--opset` 参数的 JSON 等价写法。若目标运行时要求固定的默认 ai.onnx opset 版本，可在这里直接指定。
...
| 字段 | 类型 | 默认值 | 可选值 | 说明 |
|---|---|---|---|---|
| `calibration_step` | `int` | `500` | 10–1000 | 最大校准迭代步数（dataloader 的 batch 次数），实际使用样本数约为 `calibration_step * batch_size` |
| `calibration_batch_size` | `int` | `1` | ≥ `1` | 每个校准 batch 加载的样本数量，实际使用样本数约为 `calibration_step × calibration_batch_size` |
...
| `input_parameters` | `list` | **必填** | — | 每个输入的校准设置，每个模型输入对应一项（见下方） |
...
| `dtype` | `string` | 初始为 `float32`，加载模型后可从模型中读取 | 任意 ONNX Tensor dtype | 输入数据类型。`npy` 校准数据支持 `int64` 等非浮点输入；`img` 与 `raw` 加载器按 `float32` 解释数据 |
...
对于多输入模型，每个 `input_parameters` 条目都有各自的列表文件。所有列表中**同一行号**的文件共同构成一个校准 batch，因此各列表的长度和顺序必须保持一致。
...
> **YOLO 提示：** XSlim 会先尝试把受支持的 YOLO decode 密集后处理融合成 `spacemit_functions.YoloDecode`。只有当导出图形态不匹配融合规则，或你明确希望把后处理留在量化区域之外时，再使用 `truncate_var_names` 作为回退方案。
...
XSlim 会保留输入模型里已有的 ONNX `FunctionProto` 定义；当模型中出现 `YoloDecode`、`BatchMatMul` 等融合后的 `spacemit_functions` 节点时，也会自动补齐所需 import。无需额外配置。

---

## 静态量化输入模型要求

静态 INT8 量化要求输入 ONNX 模型仍为浮点模型。若模型中已经包含 `QuantizeLinear` 或 `DequantizeLinear`，XSlim 会认为该模型已经量化，并抛出 `ValueError`，避免对同一计算图重复量化。

请使用原始浮点模型作为静态 INT8 输入。若只需要模型精简、动态量化或 FP16 转换，请显式使用对应的命令行模式。

---

## 算子覆盖说明

XSlim 2.1.0 扩展了分析与校准阶段使用的 ONNX 执行器和量化 socket 元数据覆盖范围，主要包括：

- 带可选 `axes` 输入的 opset-24 `Pad`；
- `GreaterOrEqual`、`LessOrEqual`、`Xor` 等比较与逻辑算子；
- `Celu`、`Hardmax`、`Mish`、`Softsign`、`ThresholdedRelu`、三角函数、反三角函数、双曲函数、`Round`、`Sign`、`IsInf`、`IsNaN` 等激活与一元算子；
- `ReduceL1`、`ReduceLogSum`、`ReduceLogSumExp`、`ReduceMin`、`ReduceProd`、`ReduceSumSquare` 等规约算子，并支持标量 Tensor 与通过输入 Tensor 传入 axes 的形式。

若模型中仍包含暂不支持的 ONNX 算子，XSlim 会在图加载、精简、分析或量化阶段报告问题，便于你调整导出方式、先行简化模型，或排除受影响子图。

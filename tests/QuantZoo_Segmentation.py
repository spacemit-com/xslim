...
CONFIGS = [
    {"Model": "stdc1_512x1024_80k_cityscapes", "Output": ["/convs/convs.0/activate/Relu_output_0"]},
    {"Model": "pspnet_r50-d8_512x1024_40k_cityscapes", "Output": ["/bottleneck/activate/Relu_output_0"]},
    {"Model": "pointrend_r50_512x1024_80k_cityscapes", "Output": ["/Concat_60_output_0"]},  # complex model
    {"Model": "fpn_r50_512x1024_80k_cityscapes", "Output": ["/Add_2_output_0"]},
    {"Model": "icnet_r18-d8_832x832_80k_cityscapes", "Output": ["/convs/convs.0/activate/Relu_output_0"]},
    {"Model": "fcn_unet_s5-d16_4x4_512x1024_160k_cityscapes", "Output": ["/convs/convs.0/activate/Relu_output_0"]},
    {
        "Model": "fast_scnn_lr0.12_8x4_160k_cityscapes",
        "Output": ["/convs/convs.1/pointwise_conv/activate/Relu_output_0"],
    },
    {"Model": "fcn_r50-d8_512x1024_40k_cityscapes", "Output": ["/conv_cat/activate/Relu_output_0"]},
    {"Model": "bisenetv2_fcn_4x4_1024x1024_160k_cityscapes", "Output": ["/convs/convs.0/activate/Relu_output_0"]},
    {"Model": "deeplabv3_r50-d8_512x1024_40k_cityscapes", "Output": ["/bottleneck/activate/Relu_output_0"]},
]
...
with ENABLE_CUDA_KERNEL():
    for config in CONFIGS:
        ...
        quantizer = MyInt8Quantizer(
            graph=graph, sym=SYMMETRICAL, power_of_2=POWER_OF_2, num_of_bits=BIT_WIDTH, per_channel=PERCHANNEL
        )
        if FP8:
            quantizer = MyFP8Quantizer(graph=graph, calibration="floating")
        ...
        performance = error_analyze(
            graph=graph, outputs=monitoring_vars, dataloader=test_loader, collate_fn=lambda x: x[0].cuda(), verbose=True
        )

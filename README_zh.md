...
| 版本 | 主要更新 |
| --- | --- |
| 2.1.1 | 2.1.0 发布后的当前代码树开发版本 |
| [2.1.0](https://github.com/spacemit-com/xslim/releases/tag/2.1.0) | 最新正式发布版本；新增面向受支持 YOLO 导出模型的 `spacemit_functions.YoloDecode` 自动融合，量化/导出时保留自定义 ONNX `FunctionProto` 定义，补强 opset 24 与自定义域相关处理覆盖，扩展 ONNX 算子执行与 socket 覆盖，支持标量输入及 axes 输入形式的规约算子，并拒绝对已包含 `QuantizeLinear` / `DequantizeLinear` 的模型执行静态重复量化 |
| [2.0.14](https://github.com/spacemit-com/xslim/releases/tag/2.0.14) | 为量化与转换流程新增可配置的默认 `ai.onnx` opset 转换能力 |

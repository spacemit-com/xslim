    ...
    def rewrite(self, opset=11):
        ...
        if (
            ...
            if not len(norm_scale_constant.shape) == 1 or not len(norm_bias_constant.shape) == 1:
                ...
                permute_in = [i for i in range(
                    axis)] + [i for i in range(axis + 1, ndim)] + [axis]
                permute_out = [i for i in range(
                    axis)] + [ndim - 1] + [i - 1 for i in range(axis + 1, ndim)]

                ln_input = osg.Variable("{}_ln_input".format(
                    reduce_mean_0_node.name), np.float32)
                ln_output = osg.Variable("{}_ln_output".format(
                    reduce_mean_0_node.name), np.float32)
                ...
                axis = -1
                mul_0_node.inputs[0].values = norm_scale_constant.values.reshape(
                    -1)
                add_1_node.inputs[1].values = norm_bias_constant.values.reshape(
                    -1)

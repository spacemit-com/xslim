...
class ONNXRUNTIMExporter(OnnxExporter):
    ...
    def remove_activation_ops(self, graph: BaseGraph) -> BaseGraph:
        ...
        for op in removed_activations:
            ...
            created = self.insert_quantize_node(graph=graph, var=input_var, config=config, op=op)
            self.insert_dequantize_node(graph=graph, var=created.outputs[0], config=config, op=created)
    ...
    def convert_operation(self, graph: BaseGraph, op: QuantableOperation, quantized_param: bool):
        ...
        for config, var in [_ for _ in op.config_with_variable]:
            ...
            if var.is_parameter:
                ...
                if not quantized_param:
                    created = self.insert_quantize_node(graph=graph, var=inserting_var, config=config, op=inserting)
    ...
    def export(
        ...
        onnx_model = helper.make_model(graph_def, producer_name="xslim", opset_imports=opsets)
        ...
        xslim_version = onnx_model.metadata_props.add()

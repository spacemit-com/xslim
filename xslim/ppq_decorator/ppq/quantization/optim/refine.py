...
import torch
from xslim.defs import PASSIVE_OPERATIONS
...
from ...IR.quantize import QuantableVariable
    ...
    * force_overlap(bool)
    ...
    def align_to_large(self, op: QuantableOperation) -> TensorQuantizationConfig:
        ...
        for config in op.config.input_quantization_config:
            if config.state in {QuantizationStates.FP32, QuantizationStates.SOI}:
                continue
        ...
        for operation in graph.operations.values():
            ...
            if master_config is not None:
                ...
                for up_op in graph.get_upstream_operations(operation):
                    ...
                    if len(graph.get_downstream_operations(up_op)) != 1 and not self.force_overlap:
                        continue

...
from ...executor import TorchQuantizeDelegator
from ...IR import BaseGraph, Operation, SearchableGraph, Variable
from ..qfunction import PPQuantFunction
from ...utils.fetch import batch_random_fetch
    ...
    def __call__(self, tensor: torch.Tensor, config: TensorQuantizationConfig) -> torch.Tensor:
        if config.policy.has_property(QuantizationProperty.PER_TENSOR):
            return TensorwiseRoundTuningImpl.apply(
                tensor, config.scale, config.offset, config.quant_min, config.quant_max, self._rounding
            )

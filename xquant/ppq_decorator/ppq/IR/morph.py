    ...
    def format_parameter(self) -> None:
        ...
        for var in [_ for _ in self.graph.variables.values()]:
            ...
            if var.is_parameter and len(var.dest_ops) > 1:
                for op in var.dest_ops:
                    created = self.graph.create_variable(value=var.value.clone(), is_parameter=True)
...
class GraphMerger(GraphCommandProcessor):
        ...
        def _fuse(
            ...
            if bias is not None:
                self.graph.create_link_with_op(
                    variable=self.graph.create_variable(value=bias, is_parameter=True), A=None, B=layernorm
                )
            return layernorm
    ...
    def fuse_selfattention(self):
        search_engine = SearchableGraph(graph=self.graph)
        matches = search_engine.pattern_matching(
            patterns=["MatMul", "Add", "Softmax", "MatMul"], edges=[[0, 1], [1, 2], [2, 3]], exclusive=False
        )
        ...
        for m1, add, softmax, m2 in matches:
            ...
            op = self.graph.create_operation(
                ...
                attributes={"TransQ": perm_Q, "TransK": perm_K, "TransV": perm_V, "TransO": perm_O},
        ...
        matches = search_engine.pattern_matching(
            patterns=["MatMul", "Softmax", "MatMul"], edges=[[0, 1], [1, 2]], exclusive=False
        )

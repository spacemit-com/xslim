...
from ...executor import BaseGraphExecutor
from ...IR import BaseGraph, BaseGraph, GraphFormatter, Operation, QuantableOperation, Variable
from ...IR.morph import GraphDecomposer
        ...
        with torch.no_grad():
            for name in self.interested_layers:
                ...
                if processed_values > 0:
                    ...
                    graph.create_link_with_op(variable=op1.inputs[0], A=input_var.source_op, B=op2)
    ...
    def optimize(self, graph: BaseGraph, dataloader: Iterable, executor: BaseGraphExecutor, **kwargs) -> None:
        ...
        for op in interested_ops:
            ...
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([0]), is_parameter=True), A=None, B=op5
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([hidden_size]), is_parameter=True), A=None, B=op5
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([1]), is_parameter=True), A=None, B=op5
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([1]), is_parameter=True), A=None, B=op5
            )
            ...
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([hidden_size]), is_parameter=True), A=None, B=op6
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([2 * hidden_size]), is_parameter=True), A=None, B=op6
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([1]), is_parameter=True), A=None, B=op6
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([1]), is_parameter=True), A=None, B=op6
            )
